...
class Admin:
    ...
    def admin_mainmenu(self,a,b):
         ...
         self.searchbut = Button(self.searchframe, text="Search Cus. Name", font="roboto 14", bg="#FFFFFF", bd=5, command=self.searchprod)
    ...
    def buildprodtable(self):
         ...
         self.tree = ttk.Treeview(self.tableframe, columns=("Customer ID", "Product Name", "Cus. Name", "Category",
         ...
         self.tree.heading('Cus. Name', text="Cus. Name", anchor=W)
         ...
         va = 5
         l1 = ['Product Name', 'Cus. Name', 'Category', 'Price', 'Current Stock', 'Add Stock']
         for i in range(0,6):
    ...
    def mainsearch(self, f):
        ...
        if (f==1):
            ...
            self.searchbut.config(text="Search Cus. Name",command=self.searchprod)
    ...
    def additems(self):
        ...
        l=['Customer ID',"Product Name","Cus. Name","Category","Price","Stock"]
        for i in range(0,len(l)):
        ...
        Button(self.itemframe,text="Add item",height=3,bd=6,command=self.insertitem,bg="#FFFFFF").grid(row=6,column=1,pady=10,padx=12,sticky="w",ipadx=10)
    ...
    def insertitem(self):
        ...
        if self.newitemcode.get() == '' or self.newitem.get() == '' or self.newitemdesc.get() == '':
            ...
            for i in range(0,len(l)):
                if(not l[i].isdigit()):
                    if(i==0):
                        messagebox.showerror("Error", "Customer ID should be in numeral")
                    else:
                        messagebox.showerror("Error", "Invalid Data Provided")
                    return
                elif(int(l[i])<0):
        ...
        l=self.cur.fetchall()
        if(len(l)>0):
            messagebox.showerror("Error", "Customer ID Should Be Unique")
            return
        if(self.desc_name.count(self.newitemdesc.get())!=0):
            messagebox.showerror('Error','Product with same Cus. Name exsits!')
            return
    ...
    def buildsalestable(self):
        ...
        scrollbarx = Scrollbar(self.tableframe1, orient=HORIZONTAL)
        scrollbary = Scrollbar(self.tableframe1, orient=VERTICAL)
        self.tree = ttk.Treeview(self.tableframe1, columns=("Transaction ID","Invoice No.", "Customer ID", "Cus. Name",
